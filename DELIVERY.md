...
> 状态：基础版 G1 ✅ / 升级包 G2（SQLite+FastAPI）✅ / 演示资产 G3 ✅（手算耗时改估算口径 ✅、拦截截图 ✅；演示视频经用户决策 2026-09-08 暂缓，非交付阻塞项）
> 归档日期：2026-09-08
...
### 2) 测试
- `tests/` 10 个测试文件，**49 个 pytest 全部通过**（核心模块每模块 ≥3 条用例）
...
| 五金成本 | 7,040 元 |
| **总成本** | **51,830 元** |
| 脚本 50 单耗时 | **≈ 0.09 秒**（单次 84–107 ms，本机多次实测） |
| 测试 | **49 passed** |
...
## 五、遗留事项（已全部处置完毕，2026-09-08）
- [x] ~~手算 5 单耗时实测~~ → **用户决策（2026-09-08）：演示项目采用估算口径**。README 保留「约 40 分钟」并明确标注「估算、非实测」；`手算实测工作表.md` 转为面试前自练材料（可选，不算交付项）
- [x] ~~3-5 分钟演示视频~~ → **用户决策（2026-09-08）：暂缓**。理由：演示项目以跑通流程为主；已有拦截截图 + README + 在线仓库可支撑面试讲解。若面试邀约后需要，再按 `G3_CHECKLIST.md` 第二节脚本录制（约 60~90 分钟）
- [x] ~~2 分钟 API 演示~~ → 随视频一并暂缓；面试可现场起 `uvicorn` 演示 /docs
