...
拦截证据截图（由真实运行输出渲染，可用 Cmd+Shift+4 重截原生屏摄替换）：
- `screenshots/reject_terminal.png` —— 端到端运行：50 单 → 48 合规 + 2 拦截，总成本 51,830 元
- `screenshots/reject_excel.png` —— 拦截清单明细（两颗埋雷的原因话术）
